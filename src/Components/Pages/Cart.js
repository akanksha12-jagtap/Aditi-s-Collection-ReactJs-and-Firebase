...
function Cart() {
  ...
  useEffect(() => {
    if (products.length > 0) {
      setTotal( //Updates the total state with the calculated total amount.
        products
          .map(({ id, product }) => Number(product.price*product.qty))  /*Converts each item in the products array to its corresponding price and quantity*/
          .reduce((a, b) => a + b)        //Adds up all the individual prices to get the total.
      );
    }
  }, [total, products]);
  ...
  useEffect(() => {
    if (products.length > 0) {
      setTqty( //Updates the Tqty state with the calculated total items in cart.
        products
          .map(({ id, product }) => Number(product.qty))  /*Converts each item in the products array to its corresponding quantity*/
          .reduce((a, b) => a + b)        //Adds up all the individual quantities to get the total quantity.
      );
    }
  }, [tqty, products]);
