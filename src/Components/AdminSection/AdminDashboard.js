...
function AdminDashboard() {
  ...
  const formatTimestamp = (timestamp) => {
    const dateObject = new Date(timestamp * 1000); // Assuming timestamp is in seconds, multiply by 1000 if it's in milliseconds
    const day = dateObject.toLocaleDateString(); // Format date as day
    const time = dateObject.toLocaleTimeString(); // Format time as HH:MM:SS
            ...
            <Card style={{ padding: "2rem", margin: "2rem" }} elevation={2}>
              {adminInfo.value && adminInfo.data != null ? (
                <div>
                  ...
                  <p>
                    Firm: {adminInfo.data.username} <br /> contact:{" "}
              ...
              <div style={{ padding:"1rem" }}  >
                <Grid container>
                  <Grid item xs={4}>
                    ...
                    {/*<p>
                      <b>Time: </b> {formatTimestampToDate(order.timestamp)}
                        </p>*/}
                  </Grid>
                  <Grid item xs={3}>
